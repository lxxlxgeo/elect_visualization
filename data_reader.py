...
LastEditors  : ChenKt
LastEditTime : 2023-07-27 17:46:30
    ...
    def get_time_strings(self, base_time, forecast_steps):
        ...
        for i in range(len(file_names)):
            ...
            try:
                ...
                base_time_str = (base_time + datetime.timedelta(hours=forecast_steps[i])).strftime('%m月%d日%H时')
                forecast_time_str = (base_time + datetime.timedelta(hours=forecast_steps[i] + self.forecast_step)).strftime('%m月%d日%H时')
            ...
            try:
                if self.variable_name == 'Total precipitation':
                    if i < len(grbs_list) - 1:
                        ...
                        ds_i = grbs_list[i].select(name=self.variable_name)[0]
                        # 获取gribmessage对象的值
                        ds_i_plus_1_values = ds_i_plus_1.data(lat1=latS, lat2=latN, lon1=lonL, lon2=lonR)[0] * 1000
                        ds_i_values = ds_i.data(lat1=latS, lat2=latN, lon1=lonL, lon2=lonR)[0] * 1000
                        # 进行减法运算
