...
print(data_file.sort)
for i in range(len(data_file)-1):
    print(data_file[i])
    print(data_file[i+1])
    data = xr.open_dataset(data_file[i], engine='cfgrib')

    lon = data['longitude'].loc[lonW:lonE]  # 读取经度
    lat = data['latitude'].loc[latN:latS]  # 读取纬度
# print(lat)
    cape = data["cape"][:,:].sel(latitude=slice(latN,latS),longitude=slice(lonW,lonE))
...
# print(pre)
    ...
    a = ax.contourf(lon, lat, cape,levels=np.arange(200,3001,200),cmap=cmaps.WhBlGrYeRe)
    ######获取path#######################
    ...
    plt.title('黑龙江对流有效位能预报图', fontsize=20, y=1.0)
    # plt.text(133.0, 54.3, str(time)[2:13], fontsize=12)
    cax = fig.add_axes([ax.get_position().x1+0.01,ax.get_position().y0,0.02,ax.get_position().height])
    cb = plt.colorbar(a, cax, label='对流有效位能/$J/kg$')
    # cb.set_ticks([0.1, 1, 3, 10, 20, 50])
    cb.update_ticks()  
    plt.savefig("E:/EC/heilongjiang_2023072308_"+str(step)[0:2]+"_cape.png",bbox_inches='tight', pad_inches = 0.2)