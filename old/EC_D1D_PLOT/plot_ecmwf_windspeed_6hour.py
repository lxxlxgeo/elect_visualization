...
lonW = 120.0
lonE = 135.5
data_file = glob.glob('E:/EC/2023072300_grib/*.grib1')
print(data_file)
for i in range(0,len(data_file)+1,2):
    # print(data_file[i])
    # print(data_file[i+1])
    data = xr.open_dataset(data_file[i+1], engine='cfgrib')
    # print(data.variables)
    lon = data['longitude'].loc[lonW:lonE]  # 读取经度
    lat = data['latitude'].loc[latN:latS]  # 读取纬度
# print(lat)
    p10fg6 = data["p10fg6"][:,:].sel(latitude=slice(latN,latS),longitude=slice(lonW,lonE))
...
# print(pre)
    ...
    city = r"E:/EC/heilongjiang_shp/heilongjiang_city.shp"
    ...
    city_feature = cfeature.ShapelyFeature(Reader(city).geometries(), proj, edgecolor='k', facecolor='none')
    province_feature = cfeature.ShapelyFeature(Reader(province_path).geometries(), proj, edgecolor='k', facecolor='none')
    ...
    # a = ax.contourf(lon, lat, pre, levels=[0, 0.1, 10, 25, 50, 100, 250, 500], colors=color)
    a = ax.contourf(lon, lat, p10fg6,levels=np.arange(0,35,2),cmap=cmaps.wind_17lev)
    ######获取path#######################
    ...
    plt.title('黑龙江最大风速预报图', fontsize=20, y=1.0)
    # plt.text(133.0, 54.3, str(time)[2:13], fontsize=12)
    cax = fig.add_axes([ax.get_position().x1+0.01,ax.get_position().y0,0.02,ax.get_position().height])
    cb = plt.colorbar(a, cax, label='风速/$m/s$')
    # cb.set_ticks([0.1, 1, 3, 10, 20, 50])
    cb.update_ticks()  
    plt.savefig("E:/EC/heilongjiang_2023072308_"+str(step)[0:2]+"_6hour_windspeed.png",bbox_inches='tight', pad_inches = 0.2)