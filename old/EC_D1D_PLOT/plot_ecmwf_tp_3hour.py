...
lonW = 120.0
lonE = 135.5
data_file = sorted(glob.iglob('E:/EC/2023072300_grib/*.grib1'))
# data_file = os.walk('E:/EC/2023072108/*.grib1')
print(data_file)
for i in range(len(data_file)-1):
    data1 = xr.open_dataset(data_file[i], engine='cfgrib')
    data2 = xr.open_dataset(data_file[i+1], engine='cfgrib')
    # print(data2)
    lon = data1['longitude'].loc[lonW:lonE]  # 读取经度
    lat = data1['latitude'].loc[latN:latS]  # 读取纬度
    pre1 = data1["tp"][:,:].sel(latitude=slice(latN,latS),longitude=slice(lonW,lonE))*1000
    pre2 = data2["tp"][:,:].sel(latitude=slice(latN,latS),longitude=slice(lonW,lonE))*1000
    pre = pre2-pre1
    ...
    cb.update_ticks()  
    plt.savefig("E:/EC/heilongjiang_2023072308_"+str(step)[0:2]+"_3hour_pre.png",bbox_inches='tight', pad_inches = 0.2)