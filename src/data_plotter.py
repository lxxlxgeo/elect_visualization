...
LastEditTime: 2023-08-01 17:56:24
...
# %%
class Plotter:
    def __init__(self, fpath, variable_name, variables, ticks, forecast_path_str, manual_time=None, forecast_step=3,
                 end=75, forecast_type='short-term'):
        '''
        :param fpath: 没搞懂这是啥
    ...
    def get_output_file_path(self, variable_name, index, output_prefix, file_name):
        ...
        output_path = os.path.join(output_prefix, self.forecast_type, self.forecast_path_str, variable_folder,
                                   file_name)
    ...
    def plot_contour_map(self, lat, lon, data, variable_label, levels, colormap, title, output_file, tips='colors',
        ...
        if tips == 'colors':
            a = ax.contourf(lon, lat, data, levels=levels, colors=colormap, extend='max')  # ,extend='max')
        elif tips == 'cmaps':
            a = ax.contourf(lon, lat, data, levels=levels, cmap=colormap, extend='max')  # ,extend='max')
        ...
        cb = plt.colorbar(a, cax, label=variable_label, extendrect=True)
