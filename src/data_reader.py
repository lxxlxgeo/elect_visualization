...
class GribDataReader:
    def __init__(self, fpath, variable_name, forecast_type, forecast_type_tips, manual_time=None, forecast_step=3,
                 end=75, ):
        self.fpath = fpath
    ...
    def get_forecast_steps(self, hour):
        ...
        if self.forecast_type == 'imminent':
            if hour == 11 or hour == 23:
                ...
                return list(range(21, self.end, self.forecast_step))
            elif hour == 15:
                return list(range(21, self.end, self.forecast_step))
            elif hour == 2 or hour == 14:
                return list(range(18, self.end, self.forecast_step))
    ...
    def get_base_time(self, current_time, forecast_hour):
        ...
        if self.forecast_type == 'imminent':
            if forecast_hour == 5 or forecast_hour == 11:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 12)
            elif forecast_hour == 17 or forecast_hour == 23:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 0)
            elif forecast_hour == 22:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 0)
            elif forecast_hour == 15:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 12)
            elif forecast_hour == 2 or forecast_hour == 20:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 12)
            elif forecast_hour == 8 or forecast_hour == 14:
                base_time = datetime.datetime(current_time.year, current_time.month, current_time.day, 0)
            else:
                return None
            return base_time
    ...
    def get_time_str_mdH(self, base_time, forecast_steps):
        ...
        for i in range(len(file_names)):
            ...
            try:
                ...
                base_time_str = (base_time + datetime.timedelta(hours=8) + datetime.timedelta(
                    hours=forecast_steps[i])).strftime('%m%d%H')
                forecast_time_str = (base_time + + datetime.timedelta(hours=8) + datetime.timedelta(
                    hours=forecast_steps[i] + self.forecast_step)).strftime('%m%d%H')
    ...
    def get_time_str_YmdH(self, base_time, forecast_steps):
        ...
        for i in range(len(file_names)):
            ...
            try:
                ...
                base_time_str = (base_time + datetime.timedelta(hours=8) + datetime.timedelta(
                    hours=forecast_steps[i])).strftime('%Y%m%d%H')
                if i == 0:
                    fpath_out_time_str = base_time_str
                else:
                    continue
                forecast_time_str = (base_time + + datetime.timedelta(hours=8) + datetime.timedelta(
                    hours=forecast_steps[i] + self.forecast_step)).strftime('%Y%m%d%H')
                ...
                if self.variable_name == 'Total precipitation':
                    ...
                    if i < len(grbs_list) - 1:
                        if self.forecast_type_tips == 'imminent':
                            ...
                            ds_i_values, lats, lons = avg_files(filelist, self.variable_name, lats, lons, latS, latN,
                                                                lonL, lonR)
                            # ds_i = grbs_list[i + 1].select(name=self.variable_name)[0]
                            # #print(grbs_list)
...
def get_current_time_file(current_file, d_hour=-6):
    ...
    target_file = re.sub('[1-9].grib1|[1-9][0-9].grib1|[1-9][0-9][0-9].grib1', str(count_hours),
                         current_file) + '.grib1'
