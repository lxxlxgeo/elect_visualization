...
manual_time = datetime.datetime(2023, 7, 30,5)  # 手动运行时指定的起报时刻（utc-8）
# 中期天气预报 24 小时间隔
forecast_step = 3
end = 220
forecast_type = "Imminent"  # Set the forecast type to 'short-term', 'medium-term', or 'long-term'
#output_prefix = "../output/"  # 输出图片路径前缀，命名

output_prefix='/share/data/pic_heilongjiang'   #输出的文件夹位置
output_prefix=os.path.join(output_prefix,manual_time.strftime('%Y%m%d')+"12")
